  ...
  for (i = 0; i < nargs; i++)
      ...
      if (use_fpr)
	{
	  ...
	  if (foff < REND)
		...
		{
		case 4:
		  regcache->deprecated_cooked_write (tdep->fs0_regnum + foff, val);
		  foff++;
		  break;
		case 8:
		  regcache->deprecated_cooked_write (NDS32_FD0_REGNUM + (foff >> 1), val);
		  foff += 2;
		  break;
		default:
		  /* Long double?  */
			  ...
			  struct regcache *regcache, const gdb_byte *valbuf)
{
  ...
  if (abi_use_fpr && calling_use_fpr)
    {
      if (len == 4)
	regcache->deprecated_cooked_write (tdep->fs0_regnum, valbuf);
      else if (len == 8)
	regcache->deprecated_cooked_write (NDS32_FD0_REGNUM, valbuf);
      else
	...
	{
	  regcache->deprecated_cooked_write (NDS32_R0_REGNUM, valbuf);
	}
      else if (len < 8)
	{
	  int len1, len2;
	...
	{
	  regcache->deprecated_cooked_write (NDS32_R0_REGNUM, valbuf);
	  regcache->deprecated_cooked_write (NDS32_R0_REGNUM + 1, valbuf + 4);
	}
    }
}
