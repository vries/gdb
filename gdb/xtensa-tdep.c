			   ...
			   const void *dst)
{
  ...
  for (; len > 0; len -= 4, areg++, valbuf += 4)
    {
      if (len < 4)
	regcache->deprecated_raw_write_part (areg, offset, len, valbuf);
      else
  ...
  if (return_method == return_method_struct)
    {
      ...
      regcache->deprecated_cooked_write (ARG_1ST (tdep), buf);
    }
	  ...
	  if (n < REGISTER_SIZE && byte_order == BFD_ENDIAN_BIG)
	    {
	      ...
	      regcache->deprecated_cooked_write (r, buf);
	    ...
	    while (n > 0)
	      {
		regcache->deprecated_cooked_write (r, cp);
