		       ...
		       CORE_ADDR struct_addr)
{
  ...
  for (i = 0; i < required_arg_regs; i++)
    {
      regcache->deprecated_cooked_write (ALPHA_A0_REGNUM + i,
			      arg_reg_buffer + i * ALPHA_REGISTER_SIZE);
      regcache->deprecated_cooked_write (ALPHA_FPA0_REGNUM + i,
			      arg_reg_buffer + i * ALPHA_REGISTER_SIZE);
    }
			  ...
			  const gdb_byte *valbuf)
{
  ...
  switch (valtype->code ())
	...
	case 4:
	  ...
	  regcache->deprecated_cooked_write (ALPHA_FP0_REGNUM, raw_buffer);
	  break;

	case 8:
	  regcache->deprecated_cooked_write (ALPHA_FP0_REGNUM, valbuf);
	  break;
	...
	case 16:
	  regcache->deprecated_cooked_write (ALPHA_FP0_REGNUM, valbuf);
	  regcache->deprecated_cooked_write (ALPHA_FP0_REGNUM + 1, valbuf + 8);
	  break;
		     ...
		     int regno, void *r0_r30, void *pc, void *unique)
{
  ...
  for (i = 0; i < 31; ++i)
    if (regno == i || regno == -1)
      regcache->deprecated_raw_collect (i, regs + i * 8);

  if (regno == ALPHA_PC_REGNUM || regno == -1)
    regcache->deprecated_raw_collect (ALPHA_PC_REGNUM, pc);

  if (unique && (regno == ALPHA_UNIQUE_REGNUM || regno == -1))
    regcache->deprecated_raw_collect (ALPHA_UNIQUE_REGNUM, unique);
}
		    ...
		    int regno, void *f0_f30, void *fpcr)
{
  ...
  for (i = ALPHA_FP0_REGNUM; i < ALPHA_FP0_REGNUM + 31; ++i)
    if (regno == i || regno == -1)
      regcache->deprecated_raw_collect (i, regs + (i - ALPHA_FP0_REGNUM) * 8);

  if (regno == ALPHA_FPCR_REGNUM || regno == -1)
    regcache->deprecated_raw_collect (ALPHA_FPCR_REGNUM, fpcr);
}
