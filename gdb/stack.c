...
info_frame_command_core (struct frame_info *fi, bool selected_frame_p)
{
...

  {
	...
	print_frame_args (fp_opts, func, fi, numargs, gdb_stdout);
	puts_filtered ("\n");
      }
  }
  {
...
backtrace_command (const char *arg, int from_tty)
{
  ...
  gdb::option::process_options (&arg, false, grp);
  ...
  scoped_restore restore_set_backtrace_options
    = make_scoped_restore (&curr_set_backtrace_options, &set_bt_opts);
...
static void
frame_apply_level_completer (struct cmd_list_element *ignore,
			     completion_tracker &tracker,
			     const char *text, const char */*word*/)
{
...
void
frame_apply_all_completer (struct cmd_list_element *ignore,
			   completion_tracker &tracker,
			   const char *text, const char */*word*/)
{
  frame_apply_completer (tracker, text);
...
static void
frame_apply_completer (struct cmd_list_element *ignore,
		       completion_tracker &tracker,
		       const char *text, const char */*word*/)
{
  const char *cmd = text;
...
With a negative COUNT argument, applies the command on outermost -COUNT frames.\n"
  ...
  cmd = add_prefix_cmd ("apply", class_stack, frame_apply_command,
			...
			&frame_cmd_list);
  set_cmd_completer_handle_brkchars (cmd, frame_apply_completer);
...
Usage: frame apply all [OPTION]... COMMAND\n"
  ...
  set_cmd_completer_handle_brkchars (cmd, frame_apply_all_completer);
...
LEVEL is a space-separated list of levels of frames to apply COMMAND on.\n"
  ...
  cmd = add_cmd ("level", class_stack, frame_apply_level_command,
	   ...
	   &frame_apply_cmd_list);
  set_cmd_completer_handle_brkchars (cmd, frame_apply_level_completer);
...
Usage: backtrace [OPTION]... [QUALIFIER]... [COUNT]\n\
\n\
Options:\n\
%OPTIONS%\
