			    ...
			    const gdb_byte *valbuf)
{
  ...
  if (sparc64_structure_or_union_p (type))
  ...
  else
    {
      ...
      memset (buf, 0, 8);
      memcpy (buf + 8 - len, valbuf, len);
      regcache->deprecated_cooked_write (SPARC_O0_REGNUM, buf);
    }
}
			...
			int regnum, const void *gregs)
{
  ...
  if (sparc32)
      ...
      if (regnum == SPARC32_PSR_REGNUM || regnum == -1)
	{
	  ...
	  regcache->deprecated_raw_supply (SPARC32_PSR_REGNUM, buf);
	}

      if (regnum == SPARC32_PC_REGNUM || regnum == -1)
	regcache->deprecated_raw_supply (SPARC32_PC_REGNUM,
			      regs + gregmap->r_pc_offset + 4);

      if (regnum == SPARC32_NPC_REGNUM || regnum == -1)
	regcache->deprecated_raw_supply (SPARC32_NPC_REGNUM,
			      regs + gregmap->r_npc_offset + 4);

      if (regnum == SPARC32_Y_REGNUM || regnum == -1)
	{
	  int offset = gregmap->r_y_offset + 8 - gregmap->r_y_size;
	  regcache->deprecated_raw_supply (SPARC32_Y_REGNUM, regs + offset);
	}
    }
  else
    {
      if (regnum == SPARC64_STATE_REGNUM || regnum == -1)
	regcache->deprecated_raw_supply (SPARC64_STATE_REGNUM,
			      regs + gregmap->r_tstate_offset);

      if (regnum == SPARC64_PC_REGNUM || regnum == -1)
	regcache->deprecated_raw_supply (SPARC64_PC_REGNUM,
			      regs + gregmap->r_pc_offset);

      if (regnum == SPARC64_NPC_REGNUM || regnum == -1)
	regcache->deprecated_raw_supply (SPARC64_NPC_REGNUM,
      ...
      if (regnum == SPARC64_Y_REGNUM || regnum == -1)
	{
	  ...
	  regcache->deprecated_raw_supply (SPARC64_Y_REGNUM, buf);
	}

      if ((regnum == SPARC64_FPRS_REGNUM || regnum == -1)
	  && gregmap->r_fprs_offset != -1)
	regcache->deprecated_raw_supply (SPARC64_FPRS_REGNUM,
			 ...
			 int regnum, void *gregs)
{
  ...
  if (sparc32)
      ...
      if (regnum == SPARC32_PSR_REGNUM || regnum == -1)
	{
	  ...
	  tstate = extract_unsigned_integer (regs + offset, 8, byte_order);
	  regcache->deprecated_raw_collect (SPARC32_PSR_REGNUM, buf);
      ...
      if (regnum == SPARC32_PC_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC32_PC_REGNUM,
			       regs + gregmap->r_pc_offset + 4);

      if (regnum == SPARC32_NPC_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC32_NPC_REGNUM,
			       regs + gregmap->r_npc_offset + 4);

      if (regnum == SPARC32_Y_REGNUM || regnum == -1)
	{
	  int offset = gregmap->r_y_offset + 8 - gregmap->r_y_size;
	  regcache->deprecated_raw_collect (SPARC32_Y_REGNUM, regs + offset);
	}
    }
  else
    {
      if (regnum == SPARC64_STATE_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC64_STATE_REGNUM,
			       regs + gregmap->r_tstate_offset);

      if (regnum == SPARC64_PC_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC64_PC_REGNUM,
			       regs + gregmap->r_pc_offset);

      if (regnum == SPARC64_NPC_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC64_NPC_REGNUM,
			       regs + gregmap->r_npc_offset);
      ...
      if (regnum == SPARC64_Y_REGNUM || regnum == -1)
	{
	  ...
	  regcache->deprecated_raw_collect (SPARC64_Y_REGNUM, buf);
      ...
      if ((regnum == SPARC64_FPRS_REGNUM || regnum == -1)
	  && gregmap->r_fprs_offset != -1)
	regcache->deprecated_raw_collect (SPARC64_FPRS_REGNUM,
			       regs + gregmap->r_fprs_offset);
  ...
  if ((regnum >= SPARC_G1_REGNUM && regnum <= SPARC_O7_REGNUM) || regnum == -1)
	...
	{
	  if (regnum == i || regnum == -1)
	    regcache->deprecated_raw_collect (i, regs + offset);
	  offset += 8;
	}
    }
			 ...
			 int regnum, const void *fpregs)
{
  ...
  if (sparc32)
	...
	regcache->deprecated_raw_supply (SPARC32_FSR_REGNUM,
	...
	regcache->deprecated_raw_supply (SPARC64_FSR_REGNUM,
			      regs + fpregmap->r_fsr_offset);
    }
}
			  ...
			  int regnum, void *fpregs)
{
  ...
  for (i = 0; i < 32; i++)
    {
      if (regnum == (SPARC_F0_REGNUM + i) || regnum == -1)
	regcache->deprecated_raw_collect (SPARC_F0_REGNUM + i,
			       regs + fpregmap->r_f0_offset + (i * 4));
    }
  ...
  if (sparc32)
    {
      if (regnum == SPARC32_FSR_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC32_FSR_REGNUM,
			       regs + fpregmap->r_fsr_offset);
    }
  else
    {
	...
	{
	  if (regnum == (SPARC64_F32_REGNUM + i) || regnum == -1)
	    regcache->deprecated_raw_collect (SPARC64_F32_REGNUM + i,
				   (regs + fpregmap->r_f0_offset
				    + (32 * 4) + (i * 8)));
	}

      if (regnum == SPARC64_FSR_REGNUM || regnum == -1)
	regcache->deprecated_raw_collect (SPARC64_FSR_REGNUM,
			       regs + fpregmap->r_fsr_offset);
    }
}
