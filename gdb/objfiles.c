...
{
  gdb_assert (sections_start <= a);
  gdb_assert (a <= b);
  gdb_assert (b < sections_end);

  section_iterator it (a, b);
  section_iterator end (b, b);
  while (it != end)
    {
      struct obj_section &s = *it;
      ++it;
      if (it == end)
	break;
      struct obj_section &next_s = *it;

      if (s.endaddr_unrel () < next_s.addr_unrel ())
	return true;
    }
