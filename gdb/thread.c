...
{
  return {info_thread_option_defs, it_opts};
}
				...
				const char *text, const char *word_ignored)
{
  ...
  if (gdb::option::complete_options (tracker, &text, false, grp))
    return;
...
shortcut for 'thread apply all -s frame apply all -s COMMAND'"));
  set_cmd_completer_handle_brkchars (c, frame_apply_all_completer);
