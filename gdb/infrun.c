      ...
      else
	{
	  ...
	  struct address_space *aspace;
	  {
	    /* Temporarily switch to the vfork parent, to facilitate ptrace
	       calls done during maybe_new_address_space.  We won't need to
	       do this anymore once we start using refcounting to decide
	       whether an address space is shared.  See this (
	       https://sourceware.org/pipermail/gdb-patches/2023-October/202928.html
	       ) discussion.  */
